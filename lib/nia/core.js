...
function niaOwnerDashboard(db, user) {
  ...
  const card = (key, icon, title, question, metrics, tone, insight, action) => `<article class="owner-intelligence-card ${tone}"><div class="owner-card-heading"><div class="owner-card-icon">${niaIcon(icon)}</div><div><span class="eyebrow">${title}</span><h3>${question}</h3></div><span class="owner-status ${tone}">${statusLabel(tone)}</span></div><div class="owner-metrics">${metrics.map(metric => `<div><strong>${metric.value}</strong><span>${metric.label}</span></div>`).join('')}</div><div class="owner-insight-copy"><strong>${niaIcon(iconFor(tone))} ${insight}</strong><span>Suggested action: ${action}</span></div><button class="text-action" data-owner-action="${key}">${action} ${niaIcon('arrow')}</button></article>`;
    ...
    document.getElementById('content').innerHTML = `<div class="owner-hero"><div><div class="eyebrow">Practice intelligence dashboard</div><h1>Good morning, ${firstName}</h1><p>Decision support for ${db.facility.facility_name}, ${suffix}.</p></div><div class="owner-filters"><label>Branch<select id="owner-branch"><option>All Branches</option>${branchFacilities.filter(facility => facility.facility_id !== db.facility.facility_id).map(facility => `<option>${facility.facility_name.replace(/^NiaCARE\s*(?:—|-|\|)\s*/, '')}</option>`).join('')}</select></label><label>Period<select id="owner-period"><option>Week</option><option selected>Month</option><option>Quarter</option><option>Year</option></select></label></div></div>
      ...
      <div class="owner-intelligence-grid owner-top-signals">${card('treatment-plans', 'plans', 'Treatment performance', 'Conversions & Backlog', [{ value: money(pendingValue), label: 'pending value' }, { value: `${treatmentAcceptance}%`, label: 'acceptance rate' }, { value: `${plans.length}`, label: 'overdue plans' }], treatmentTone, `${treatmentAcceptance}% treatment acceptance from ${plans.length} plans.`, 'Review Treatment')}${card('appointments', 'appointments', 'Follow-up & appointments', 'Conversion Dynamics', [{ value: `${followUpRate}%`, label: 'follow-up contact' }, { value: `${appointmentRate}%`, label: 'appt conversion' }, { value: `${noShowRate}%`, label: 'no-show rate' }], followTone, `${followUpRate}% of follow-ups have a recorded outcome.`, 'Review Reminders')}${card('recalls', 'calendar', 'Recall performance', 'Ongoing Care Continuity', [{ value: '—', label: 'recall booking' }, { value: '—', label: 'recall completion' }, { value: '—', label: 'overdue recalls' }], recallTone, 'Recall data is ready to be monitored.', 'Review Recalls')}${card('feedback', 'heart', 'Patient experience', 'Feedback & Sentiment', [{ value: '—', label: 'satisfaction' }, { value: '—', label: 'resolution rate' }, { value: '—', label: 'open complaints' }], experienceTone, 'Patient feedback signals are currently within target.', 'View Feedback')}${card('incidents', 'shield', 'Patient safety', 'Incidents & Audits', [{ value: '0', label: 'serious incidents' }, { value: '100%', label: 'closure rate' }, { value: 'None', label: 'recurring patterns' }], safetyTone, 'No serious patient safety incidents recorded.', 'View Incidents')}</div>
