...
function buildSeedData(){
  ...
  const users = [
    ...
    { user_id: 'u_owner_neema', facility_id: 'fac_001', full_name: 'Neema Mwadime', email: 'neema.mwadime@niahealthcare.co.ke', role: 'owner', active_status: true, created_at: '2024-02-10' },
...
function niaOwnerDashboard(db, user){
  ...
  const insights = [
    { tone:'warning', icon:'bell', title:'Treatment drop-off needs attention', body:'7 patients have accepted plans with no appointment booked in the last 14 days.', action:'Review drop-off' },
    { tone:'danger', icon:'shield', title:'2 safety incidents remain open', body:'Both incidents are assigned and due for review this week.', action:'View incidents' },
    { tone:'success', icon:'check', title:'Patient experience is improving', body:'Feedback score is up 8% compared with the previous period.', action:'View feedback' },
  ];
  const navCard = (icon, label, value, meta, tone='') => `<div class="owner-kpi ${tone}"><div class="owner-kpi-top"><span class="owner-kpi-icon">${niaIcon(icon)}</span><span class="owner-kpi-meta">${meta}</span></div><strong>${value}</strong><span>${label}</span></div>`;
  const render = (period='Month', branch='All Branches') => {
    const suffix = branch === 'All Branches' ? 'across all branches' : `at ${branch}`;
    document.getElementById('content').innerHTML = `
      <div class="owner-hero"><div><div class="eyebrow">Practice intelligence dashboard</div><h1>Good morning, ${user.full_name.replace(/^(Dr\\.|Mr\\.|Mrs\\.|Ms\\.)\\s+/,'').split(' ')[0]}</h1><p>Here is what needs your attention ${suffix}.</p></div><div class="owner-filters"><label>Branch<select id="owner-branch"><option>All Branches</option><option>Nyali Dental Centre</option><option>Kileleshwa Smile Studio</option></select></label><label>Period<select id="owner-period"><option>Week</option><option selected>Month</option><option>Quarter</option><option>Year</option></select></label></div></div>
      <div class="owner-health"><div><span class="eyebrow">Overall practice health</span><div class="health-row"><span class="health-dot"></span><strong>Healthy, with attention needed</strong></div><p>Core operations are stable. Focus on treatment continuity and open safety actions.</p></div><div class="health-score"><strong>82</strong><span>/ 100</span><small>+6 vs last ${period.toLowerCase()}</small></div></div>
      <div class="owner-section-head"><div><span class="eyebrow">At a glance</span><h2>Key performance signals</h2></div><button class="btn btn-ghost btn-sm" id="owner-export">${niaIcon('reports')} Export summary</button></div>
      <div class="owner-kpi-grid">${navCard('money','Revenue collected','KES 4.82M','+12.4% vs prior period','positive')}${navCard('patients','Active patients','1,248','+84 this month','positive')}${navCard('appointments','Appointment completion','91%','-2.1% vs prior period','warning')}${navCard('plans','Treatment acceptance','68%','+4.8% vs prior period','positive')}</div>
      <div class="owner-main-grid"><section class="card owner-panel"><div class="card-header"><div><h3>Performance trend</h3><div class="hint">Revenue and completed appointments · ${period}</div></div><span class="owner-legend"><i></i> Revenue <i class="blue"></i> Appointments</span></div><div class="owner-chart"><div class="chart-y"><span>6M</span><span>4M</span><span>2M</span><span>0</span></div><div class="chart-area"><svg viewBox="0 0 700 210" preserveAspectRatio="none" role="img" aria-label="Performance trend rising over the selected period"><path class="chart-fill" d="M0,170 C70,150 100,165 150,130 S245,145 300,105 S400,125 450,80 S560,92 700,35 L700,210 L0,210 Z"></path><path class="chart-line" d="M0,170 C70,150 100,165 150,130 S245,145 300,105 S400,125 450,80 S560,92 700,35"></path><path class="chart-line secondary" d="M0,184 C80,175 110,182 160,160 S250,172 310,145 S420,155 470,125 S580,137 700,105"></path></svg><div class="chart-x"><span>Jan</span><span>Feb</span><span>Mar</span><span>Apr</span><span>May</span><span>Jun</span></div></div></div></section><section class="card owner-panel"><div class="card-header"><div><h3>Branch performance</h3><div class="hint">Compared with previous period</div></div><button class="icon-btn" aria-label="More branch options">${niaIcon('more')}</button></div><div class="branch-row"><div><strong>Nyali Dental Centre</strong><span>82% health score</span></div><b>+14%</b></div><div class="branch-progress"><i style="width:82%"></i></div><div class="branch-row"><div><strong>Kileleshwa Smile Studio</strong><span>76% health score</span></div><b>+8%</b></div><div class="branch-progress"><i style="width:76%"></i></div><button class="btn btn-ghost btn-sm owner-full-width">View branch comparison</button></section></div>
      <div class="owner-section-head insights-head"><div><span class="eyebrow">Recommended next steps</span><h2>What needs your attention</h2></div><span class="owner-count">3 open insights</span></div><div class="owner-insights">${insights.map(item => `<article class="owner-insight ${item.tone}"><span class="insight-icon">${niaIcon(item.icon)}</span><div><h3>${item.title}</h3><p>${item.body}</p><button class="text-action">${item.action} ${niaIcon('arrow')}</button></div></article>`).join('')}</div>`;
