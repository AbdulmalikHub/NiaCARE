...
function niaOwnerDashboard(db, user) {
  ...
  const card = (key, icon, eyebrow, title, metrics, tone, insight, action) => { const normalizedMetrics = metrics; const visibleMetrics = normalizedMetrics.length > 5 ? normalizedMetrics.slice(0, 5) : normalizedMetrics; const extraMetrics = normalizedMetrics.length > 5 ? normalizedMetrics.slice(5) : []; return `<article class="owner-intelligence-card ${tone}${extraMetrics.length ? ' is-collapsible' : ''}" data-card-key="${key}"><div class="owner-card-heading"><div class="owner-card-icon">${niaIcon(icon)}</div><div class="owner-card-title"><span class="eyebrow">${eyebrow}</span><h3>${title}</h3><p>${action}</p></div>${extraMetrics.length ? `<details class="owner-card-dropdown"><summary aria-label="Show all ${title} displays"><span aria-hidden="true">⌄</span></summary><div class="owner-card-details" id="owner-details-${key}">${extraMetrics.map(metric => `<div class="owner-metric-row"><span class="owner-metric-icon">${niaIcon(metric.icon || (metric.label.includes('follow-up') ? 'users' : metric.label.includes('appointment') ? 'calendar' : metric.label.includes('reminder') ? 'bell' : metric.label.includes('no-show') ? 'alert' : metric.label.includes('cancellation') ? 'x' : metric.label.includes('treatment') ? 'activity' : 'chart'))}</span><span class="owner-metric-label">${metric.label}</span><strong>${metric.value}</strong><i class="metric-dot ${metric.tone || tone}" aria-hidden="true"></i></div>`).join('')}</div></details>` : ''}</div><div class="owner-metrics">${visibleMetrics.map(metric => `<div class="owner-metric-row"><span class="owner-metric-icon">${niaIcon(metric.icon || (metric.label.includes('follow-up') ? 'users' : metric.label.includes('appointment') ? 'calendar' : metric.label.includes('reminder') ? 'bell' : metric.label.includes('no-show') ? 'alert' : metric.label.includes('cancellation') ? 'x' : metric.label.includes('treatment') ? 'activity' : 'chart'))}</span><span class="owner-metric-label">${metric.label}</span><strong>${metric.value}</strong><i class="metric-dot ${metric.tone || tone}" aria-hidden="true"></i></div>`).join('')}</div>${extraMetrics.length ? `<details class="owner-card-dropdown"><summary>View all displays <span aria-hidden="true">⌄</span></summary><div class="owner-card-details" id="owner-details-${key}">${extraMetrics.map(metric => `<div><span>${metric.label}</span><strong>${metric.value}</strong><i class="metric-dot ${metric.tone || tone}" aria-hidden="true"></i></div>`).join('')}</div></details>` : ''}${metrics.some(metric => metric.trend) ? `<div class="owner-card-trend ${metrics.find(metric => metric.trend)?.trendTone || ''}">${metrics.find(metric => metric.trend)?.trend || ''}</div>` : ''}<div class="owner-insight-copy ${tone}"><strong>${niaIcon('info')} ${insight}</strong></div><button class="text-action" data-owner-action="${key}">${action} ${niaIcon('arrow')}</button></article>`; };
