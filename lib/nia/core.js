...
function niaOwnerDashboard(db, user) {
  ...
  const render = (period = 'Month', branch = 'All Branches') => {
    const plans = niaScopedPlans(db); const appointments = niaScopedAppointments(db); const followUps = niaScopedFollowUps(db); const procedures = niaScopedProcedures(db); const revenue = (db.subscription?.payments || db.facility?.subscription?.payments || []).reduce((sum, payment) => sum + Number(payment.amount_usd || 0), 0);
    const patients = niaScopedPatients(db); const accepted = plans.filter(p => ['Accepted', 'In Progress', 'Completed'].includes(p.status));
