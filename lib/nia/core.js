...
function niaOwnerDashboard(db, user) {
  ...
  const card = (key, icon, eyebrow, title, metrics, tone, insight, action) => { const normalizedMetrics = metrics; const visibleMetrics = normalizedMetrics; const extraMetrics = []; return `<article class="owner-intelligence-card ${tone}${extraMetrics.length ? ' is-collapsible' : ''}" data-card-key="${key}"><div class="owner-card-heading"><div class="owner-card-icon">${niaIcon(icon)}</div><div class="owner-card-title"><span class="eyebrow">${eyebrow}</span><h3>${title}</h3></div>${extraMetrics.length ? `<details class="owner-card-dropdown"><summary>View all displays <span aria-hidden="true">⌄</span></summary><div class="owner-card-details" id="owner-details-${key}">${extraMetrics.map(metric => `<div><span>${metric.label}</span><strong>${metric.value}</strong><i class="metric-dot ${metric.tone || tone}" aria-hidden="true"></i></div>`).join('')}</div></details>` : ''}</div><div class="owner-metrics">${visibleMetrics.map(metric => `<div><span>${metric.label}</span><strong>${metric.value}</strong><i class="metric-dot ${metric.tone || tone}" aria-hidden="true"></i></div>`).join('')}</div>${metrics.some(metric => metric.trend) ? `<div class="owner-card-trend ${metrics.find(metric => metric.trend)?.trendTone || ''}">${metrics.find(metric => metric.trend)?.trend || ''}</div>` : ''}<div class="owner-insight-copy ${tone}"><strong>${niaIcon('info')} ${insight}</strong></div><button class="text-action" data-owner-action="${key}">${action} ${niaIcon('arrow')}</button></article>`; };
...
Pages.team = function (db, user) {
  ...
  db.staffInvitations = Array.isArray(db.staffInvitations) ? db.staffInvitations : [];
  const invitationFor = userId => db.staffInvitations.find(invite => invite.user_id === userId);
  const inviteLink = invite => `${window.location.origin}${window.location.pathname}#staff-signup=${encodeURIComponent(invite.token)}`;
  ...
  function render() {
    const html = `
      ...
      <div class="card mt-16">
        <div class="table-wrap">
          <table class="table">
            <thead><tr><th>Name</th><th>Email</th><th>Role</th><th>Account link</th><th>Status</th><th>Joined</th><th></th></tr></thead>
            <tbody>
              ${scopedUsers.map(u => `
                <tr>
                  ...
                  <td>${(() => { const invite = invitationFor(u.user_id); return invite ? `<div class="staff-invite-actions">${invite.status !== 'used' ? `<button class="invite-copy-icon" type="button" data-invite="${invite.token}" title="Copy account creation link" aria-label="Copy account creation link">${niaIcon('clipboard')}</button><a class="btn btn-ghost btn-sm" href="mailto:${u.email}?subject=${encodeURIComponent('Create your NiaCARE account')}&body=${encodeURIComponent(`Create your account here: ${inviteLink(invite)}`)}">Send link</a>` : ''}<span class="badge ${invite.status === 'used' ? 'badge-accepted' : 'badge-pending'}">${invite.status === 'used' ? 'Used' : 'Pending'}</span></div>` : '<span class="form-hint">Not sent</span>'; })()}</td>
    ...
    document.querySelectorAll('.invite-copy-icon').forEach(btn => btn.addEventListener('click', async () => { const invite = db.staffInvitations.find(item => item.token === btn.dataset.invite); if (!invite) return; await navigator.clipboard.writeText(inviteLink(invite)); niaToast('Account creation link copied.'); }));
  ...
  function openAddModal() {
      ...
      <div class="modal">
        ...
        <div class="modal-body">
          <div class="form-grid">
            ...
            <div class="form-field full"><p class="form-hint">After adding staff, a one-time account creation link will appear in the table. Use the copy icon or Send link to deliver it separately. The staff member creates and confirms only their password; the link is marked Used after completion.</p></div>
          </div>
        </div>
        <div class="modal-footer">
          <button class="btn btn-ghost" onclick="this.closest('.modal-backdrop').remove()">Cancel</button>
    ...
    document.getElementById('save-user').addEventListener('click', () => {
      ...
      const staffId = uid('u');
      const invite = { invitation_id: uid('invite'), user_id: staffId, facility_id: db.facility.facility_id, token: uid('signup'), status: 'pending', created_at: todayISO(), used_at: null };
      db.users.push({ user_id: staffId, facility_id: db.facility.facility_id, full_name: name, email, role: document.getElementById('nu-role').value, active_status: true, created_at: todayISO(), account_status: 'pending' });
      db.staffInvitations.push(invite);
      niaSaveDB(db);
      modal.remove();
