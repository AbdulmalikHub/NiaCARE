  ...
  const facilities = [
    {
      ...
      subscription: niaBuildSubscription('2026-05-18', 1, 'active'), // due 2026-06-17 -> 2 days overdue today = grace period
    },
    {
      facility_id: 'fac_002',
...
function niaOwnerDashboard(db, user) {
  ...
  const render = (period = 'Month', branch = 'All Branches') => {
    ...
    document.getElementById('content').innerHTML = `<div class="owner-hero"><div><div class="eyebrow">Practice intelligence dashboard</div><h1>Good morning, ${firstName}</h1><p>Decision support for ${db.facility.facility_name}, ${suffix}.</p></div><div class="owner-filters"><label>Branch<select id="owner-branch"><option>All Branches</option><option>Nyali Dental Centre</option><option>Kileleshwa Smile Studio</option></select></label><label>Period<select id="owner-period"><option>Week</option><option selected>Month</option><option>Quarter</option><option>Year</option></select></label></div></div>
      <section class="owner-health ${healthTone}"><div><span class="eyebrow">Overall practice health</span><div class="health-row"><span class="health-dot"></span><strong>${healthTone === 'success' ? 'Practice is healthy' : healthTone === 'warning' ? 'Practice needs attention' : 'Action required'}</strong></div><p>${healthTone === 'success' ? 'Core performance signals are within target.' : 'Treatment continuity and follow-up need leadership attention.'}</p></div><div class="health-score"><strong>${healthTone === 'success' ? 92 : healthTone === 'warning' ? 78 : 56}</strong><span>/ 100</span><small>${period} view</small></div></section>
      ...
      <div class="owner-overview-grid"><section class="card owner-performance"><div class="owner-panel-heading"><div><h3>Performance trend</h3><p>Revenue and completed appointments · ${period}</p></div><div class="owner-legend"><span class="revenue-dot"></span> Revenue <span class="appointments-dot"></span> Appointments</div></div><div class="owner-chart"><div class="chart-axis"><span>6M</span><span>4M</span><span>2M</span><span>0</span></div><div class="chart-area"><i style="height:26%"></i><i style="height:34%"></i><i style="height:42%"></i><i style="height:38%"></i><i style="height:57%"></i><i style="height:72%"></i><i style="height:88%"></i></div></div></section><section class="card owner-branch-performance"><div class="owner-panel-heading"><div><h3>Branch performance</h3><p>Compared with previous period</p></div></div><div class="branch-row"><div><strong>Nyali Dental Centre</strong><span>82% health score</span></div><b>+14%</b><div class="branch-meter"><i style="width:82%"></i></div></div><div class="branch-row"><div><strong>Kileleshwa Smile Studio</strong><span>74% health score</span></div><b>+8%</b><div class="branch-meter"><i style="width:74%"></i></div></div></section></div>
