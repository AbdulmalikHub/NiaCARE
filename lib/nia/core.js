...
function niaOwnerDashboard(db, user) {
  ...
  const card = (key, icon, eyebrow, title, metrics, tone, insight, action) => `<article class="owner-intelligence-card ${tone}"><div class="owner-card-heading"><div class="owner-card-icon">${niaIcon(icon)}</div><div class="owner-card-title"><span class="eyebrow">${eyebrow}</span><h3>${title}</h3></div></div><div class="owner-metrics">${metrics.map(metric => `<div><span>${metric.label}</span><strong>${metric.value}</strong><i class="metric-dot ${tone}" aria-hidden="true"></i></div>`).join('')}</div><div class="owner-insight-copy"><strong>${niaIcon('info')} ${insight}</strong></div><button class="text-action" data-owner-action="${key}">${action} ${niaIcon('arrow')}</button></article>`;
    ...
    const treatmentTone = statusFor(treatmentAcceptance, 75, 60); const followTone = statusFor(followUpRate, 90, 75); const recallRecords = scopedDb.recalls || scopedDb.recall_records || [];
    const recallBookingRate = pct(recallRecords.filter(r => ['Booked', 'Completed'].includes(r.status)).length, recallRecords.length); const recallCompletionRate = pct(recallRecords.filter(r => r.status === 'Completed').length, recallRecords.length); const overdueRecalls = recallRecords.filter(r => r.due_date && r.due_date < todayISO() && r.status !== 'Completed').length; const recallTone = recallRecords.length ? statusFor(recallBookingRate, 80, 65) : 'warning';
    ...
    const healthTone = [treatmentTone, followTone, recallTone, experienceTone, safetyTone].includes('danger') ? 'danger' : [treatmentTone, followTone, recallTone, experienceTone, safetyTone].includes('warning') ? 'warning' : 'success';
    const healthScore = Math.max(0, Math.round(([treatmentTone, followTone, recallTone, experienceTone, safetyTone].reduce((score, tone) => score + (tone === 'success' ? 20 : tone === 'warning' ? 12 : 5), 0))));
    const healthLabel = healthTone === 'success' ? 'Healthy' : healthTone === 'warning' ? 'Needs attention' : 'Action required';
    const healthInsight = healthTone === 'success' ? 'Core practice signals are within target.' : healthTone === 'warning' ? 'Some practice signals need review before they affect performance.' : 'Critical practice signals require owner review.';
    ...
    document.getElementById('content').innerHTML = `<div class="owner-hero"><div><div class="eyebrow">Practice intelligence dashboard</div><h1>Good morning, ${firstName}</h1><p>Decision support for ${db.facility.facility_name}, ${suffix}.</p></div><div class="owner-filters"><label>Branch<select id="owner-branch"><option>All Branches</option>${branchFacilities.filter(facility => facility.facility_id !== db.facility.facility_id).map(facility => `<option>${facility.facility_name.replace(/^NiaCARE\s*(?:—|-|\|)\s*/, '')}</option>`).join('')}</select></label><label>Period<select id="owner-period"><option>Week</option><option selected>Month</option><option>Quarter</option><option>Year</option></select></label></div></div>
      <section class="owner-health ${healthTone}"><div><span class="eyebrow">Overall practice health</span><div class="health-row"><span class="health-dot"></span><strong>${healthLabel}</strong></div><p>${healthInsight}</p></div><div class="health-score"><strong>${healthScore}</strong><span>/ 100</span><small>Calculated from ${[treatmentTone, followTone, recallTone, experienceTone, safetyTone].length} leadership signals</small></div></section>
