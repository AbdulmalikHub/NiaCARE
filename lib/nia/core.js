...
Pages.team = function (db, user) {
  ...
  function render() {
            ...
            <tbody>
              ${scopedUsers.map(u => `
                <tr>
                  ...
                  <td>${(() => { const invite = invitationFor(u.user_id); return invite ? `<div class="staff-invite-actions"><span class="badge ${invite.status === 'used' ? 'badge-accepted' : 'badge-pending'}">${invite.status === 'used' ? 'Used' : 'Pending'}</span>${invite.status !== 'used' ? `<button class="btn btn-ghost btn-sm copy-invite" data-invite="${invite.token}">Copy link</button><a class="btn btn-ghost btn-sm" href="mailto:${u.email}?subject=${encodeURIComponent('Create your NiaCARE account')}&body=${encodeURIComponent(`Create your account here: ${inviteLink(invite)}`)}">Email link</a>` : ''}</div>` : '<span class="form-hint">Not sent</span>'; })()}</td>
    ...
    document.querySelectorAll('.copy-invite').forEach(btn => btn.addEventListener('click', async () => { const invite = db.staffInvitations.find(item => item.token === btn.dataset.invite); if (!invite) return; await navigator.clipboard.writeText(inviteLink(invite)); niaToast('Account creation link copied.'); }));
      ...
      <div class="modal">
        ...
        <div class="modal-body">
          <div class="form-grid">
            ...
            <div class="form-field full"><p class="form-hint">After adding staff, a one-time account creation link will appear in the table. Use Copy link or Email link to send it separately. The staff member creates and confirms only their password; the link is marked Used after completion.</p></div>
          </div>
        </div>
        <div class="modal-footer">
          <button class="btn btn-ghost" onclick="this.closest('.modal-backdrop').remove()">Cancel</button>
