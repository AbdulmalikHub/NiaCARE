...
function niaOwnerDashboard(db, user) {
  ...
  const card = (key, icon, eyebrow, title, metrics, tone, insight, action) => `<article class="owner-intelligence-card ${tone}"><div class="owner-card-heading"><div class="owner-card-icon">${niaIcon(icon)}</div><div class="owner-card-title"><span class="eyebrow">${eyebrow}</span><h3>${title}</h3></div></div><div class="owner-metrics">${metrics.map(metric => `<div><span>${metric.label}</span><strong>${metric.value}</strong><i class="metric-dot ${metric.tone || tone}" aria-hidden="true"></i></div>`).join('')}</div>${metrics.some(metric => metric.trend) ? `<div class="owner-card-trend ${metrics.find(metric => metric.trend)?.trendTone || ''}">${metrics.find(metric => metric.trend)?.trend || ''}</div>` : ''}<div class="owner-insight-copy"><strong>${niaIcon('info')} ${insight}</strong></div><button class="text-action" data-owner-action="${key}">${action} ${niaIcon('arrow')}</button></article>`;
    ...
    const ownerRoutes = { 'treatment-plans': 'plans', appointments: 'appointments', 'no-shows': 'appointments', recalls: 'followups', feedback: 'reports', incidents: 'reports', reports: 'reports', 'branch-comparison': 'reports', 'treatment-performance': 'plans' }; document.querySelectorAll('[data-owner-action]').forEach(button => button.onclick = () => { const page = ownerRoutes[button.dataset.ownerAction] || 'reports'; go(page); });
    bindNavLinks();
  };
  render();
}
